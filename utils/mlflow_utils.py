...
class MLflowTracker:
    ...
    def log_stage_metrics(self, df: DataFrame, stage: str, additional_metrics: Optional[Dict] = None):
        """Log metrics for any pipeline stage directly from PySpark DataFrame."""
        try:
            # Spark session is already inside df, no need to pass separately
            missing_counts = [df.filter(F.col(col).isNull()).count() for col in df.columns]
